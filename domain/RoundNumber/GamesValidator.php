...
final class GamesValidator
{
    ...
    public function validate(
        ...
        $this->validateThatAllPlacesPlaySameNrOfGames($roundNumber);
    ...
    protected function gameCountersOfEveryPlaceAreTheSame(Poule $poule, CompetitionSport $competitionSport): bool
    {
        ...
        foreach ($poule->getGames($competitionSport) as $game) {
            ...
            foreach ($places as $place) {
                $uniquePlaceIdx = $place->getUniqueIndex();
                if (array_key_exists($uniquePlaceIdx, $nrOfGames) === false) {
                    $nrOfGames[$uniquePlaceIdx] = 0;
                }
                $nrOfGames[$uniquePlaceIdx]++;
            }
        }
