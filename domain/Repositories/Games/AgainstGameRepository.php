...
use Sports\Repositories\Games\GameRepositoryAbstract as GameRepository;
 ...
 */
class AgainstGameRepository extends GameRepository
{
     ...
     * @param GameState|null $gameStates
    ...
    public function getGameRoundState(Competition $competition, int $gameRoundNr): GameState
    {
        ...
        $created = array_filter($againstGames, function (AgainstGame $againstGame): bool {
            return $againstGame->getState() === GameState::Created;
        });
        $finished = array_filter($againstGames, function (AgainstGame $againstGame): bool {
            return $againstGame->getState() === GameState::Finished;
        });
        ...
        if (count($created) === 0 && count($finished) > 0) {
            $canceled = array_filter($againstGames, function (AgainstGame $againstGame): bool {
                return $againstGame->getState() === GameState::Canceled;
            });
